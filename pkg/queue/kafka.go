...
func ConnectProducer(brokerUrls []string, apiKey, secret string) (sarama.SyncProducer, error) {
	...
	if err != nil {
		log.Printf("Error Failed to connect to Kafka: %v\n", err)
...
func PushMessageWithKeyToQueue(brokerUrl []string, apiKey, secret, topic, key string, message []byte) error {
	...
	if err != nil {
		log.Printf("Error Failed to connect to producer: %v\n", err)
...
func ConnectConsumer(brokerUrls []string, apiKey, secret string) (sarama.Consumer, error) {
	...
	if err != nil {
		log.Printf("Error Failed to connect to consumer: %v\n", err)
...
func DecodeMessage(obj any, value []byte) error {
	...
	if err := validate.Struct(obj); err != nil {
		log.Printf("Error Failed to validate message: %v\n", err)
